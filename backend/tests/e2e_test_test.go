...
func TestEstablishmentCreationE2E(t *testing.T) {
	...
	if !isServerRunning() {
		t.Skip("Server is not running. Start the server before running e2e tests.")
	}
...
func submitOnboardingAnswers(t *testing.T, testData *TestData) {
	...
	if err != nil {
		t.Fatalf("Failed to submit onboarding answers: %v", err)
	}
	if resp.StatusCode != http.StatusCreated {
...
func getEstablishment(t *testing.T, testData *TestData) {
	...
	if http.StatusOK != resp.StatusCode { t.Fatalf("Expected %v, got %v", http.StatusOK, resp.StatusCode) }
...
func createIngredientCategory(t *testing.T, testData *TestData) {
	...
	resp, err := client.Do(req)
	if err != nil { t.Fatalf("Error: %v", err) }
	if http.StatusCreated != resp.StatusCode { t.Fatalf("Expected %v, got %v", http.StatusCreated, resp.StatusCode) }
...
// go test -v -run TestEstablishmentCreationE2E ./e2e_test_test.go
