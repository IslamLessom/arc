...
import { translateUnit } from '../lib/unitTranslator'
    ...
    <Styled.PageContainer>
        ...
        <Styled.FormSection>
          ...
          <Styled.FormRow>
            <Styled.FormField>
              ...
              <Styled.Select value={formData.supplier_id} onChange={e => handleFieldChange('supplier_id', e.target.value)}>
              ...
              {fieldErrors.supplier_id && <Styled.ErrorMessage>{fieldErrors.supplier_id}</Styled.ErrorMessage>}
            </Styled.FormField>

            <Styled.FormField>
              <Styled.Label>Склад</Styled.Label>
              <Styled.Select value={formData.warehouse_id} onChange={e => handleFieldChange('warehouse_id', e.target.value)}>
              ...
              {fieldErrors.warehouse_id && <Styled.ErrorMessage>{fieldErrors.warehouse_id}</Styled.ErrorMessage>}
            </Styled.FormField>
          </Styled.FormRow>
          ...
          <Styled.FormField>
            ...
            {formData.payments.length === 0 ? (
              <div style={{ display: 'flex', gap: '12px' }}>
                <Styled.AddPaymentButton onClick={addPayment}>+ Добавить платеж</Styled.AddPaymentButton>
              ...
              <Styled.PaymentsContainer>
                ...
                {formData.payments.map(payment => (
                  <Styled.PaymentRow key={payment.id}>
                    ...
                    <Styled.PaymentDeleteButton onClick={() => removePayment(payment.id)}>🗑️</Styled.PaymentDeleteButton>
                  </Styled.PaymentRow>
                ))}
                <Styled.AddPaymentButton onClick={addPayment}>+ Добавить платеж</Styled.AddPaymentButton>
              </Styled.PaymentsContainer>
        ...
        <Styled.FormSection>
          <Styled.ImportBox>
            ...
            <Styled.ImportContent>
              ...
              <Styled.ImportText>
                Нажмите на поле для выбора файла или перетащите в него таблицу в формате CSV, XLS, XLSX. Проверьте,
                совпадают ли названия товаров и ингредиентов в ARCE и в таблице для импорта. Добавьте новые продукты во
                вкладках <Styled.ImportLink to="/menu/products">Меню → Товары</Styled.ImportLink> и{' '}
            ...
            {formData.items.map(item => {
              ...
              return (
                <Styled.ItemRow key={item.id}>
                  ...
                  <div>
                    <Styled.ItemSelect value={item.unit} onChange={e => updateItem(item.id, { unit: e.target.value })}>
                  ...
                  <div>
                    <Styled.DeleteButton onClick={() => removeItem(item.id)}>🗑️</Styled.DeleteButton>
                  </div>
                </Styled.ItemRow>
              )
            })}
      ...
      {showInsufficientFundsModal && (
        ...
        >
          ...
          >
            <h3 style={{ marginTop: 0, marginBottom: '16px', fontSize: '18px', fontWeight: 600 }}>Недостаточно средств</h3>
            <p style={{ marginBottom: '24px', color: '#6b7280' }}>
              На выбранных счетах недостаточно средств для оплаты поставки. Вы можете оформить поставку в долг или
