...
export const useAddSupply = () => {
  ...
  const error =
    createSupplyMutation.error || updateSupplyMutation.error
      ? (createSupplyMutation.error as Error)?.message || (updateSupplyMutation.error as Error)?.message
      : null
  ...
  const isFormValid = useMemo(() => {
    return (
      ...
      formData.items.every(item => (item.ingredient_id || item.product_id) && item.quantity > 0 && item.unit.trim().length > 0) &&
      Object.keys(fieldErrors).length === 0
    )
  }, [formData, fieldErrors])

  const validateField = useCallback(
    (field: keyof AddSupplyFormData, value: string) => {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      const supplyData = {
        ...
        payment_status: payments.length === 0 ? 'none' : undefined,
      }
  ...
  const availableItems = useMemo(() => {
    ...
    const warehouseStock = formData.warehouse_id ? stock.filter(s => s.warehouse_id === formData.warehouse_id) : stock
