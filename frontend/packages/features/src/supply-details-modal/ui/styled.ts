...
export const ActionButton = styled.button<{ $variant?: 'primary' | 'secondary' | 'danger' | 'highlight' }>((props) => ({
  padding: '8px 16px',
  borderRadius: '8px',
  fontSize: '13px',
  fontWeight: 500,
  ...
  backgroundColor: props.$variant === 'primary'
    ...
    : props.$variant === 'danger'
      ...
      : 'transparent',
  color: props.$variant === 'primary' || props.$variant === 'danger' || props.$variant === 'highlight'
    ? '#fff'
...
export const PaymentAccountSelect = styled.select({
  minWidth: '240px',
  maxWidth: '280px',
  padding: '8px 10px',
  borderRadius: '8px',
...
export const PaymentErrorText = styled.div({
  marginBottom: '12px',
  fontSize: '13px',
  color: ({ theme }) => theme.colors.danger,
})
